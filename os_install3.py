...
def os_verify(script):
    ...
    if d in b:
        script.log.info("OS installation  verified")
    else:
        script.error("OS installtion error")
    if "ATA" in val5:
        script.log.info("OS installation on Hard drive verified")
    elif val6 == "FC":
        script.log.info("OS installation on FC verified")
...
def memory_match(script):
    ...
    par = script.par
    console = par.get_console_conn()
    # par.get_to_linux("Ret Hat Enterprise Linux Server")
...
def get_os_resource(script):
        ...
        if str(info_option) == '-h':
             script.log.info(
                 'get_os_resources.py [--host HOST] [--proto proto_name] [--password password] [--get_info option]'
